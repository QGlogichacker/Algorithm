...
print("np.array(List)转换为一维数组")
print(arr1)
print("arr.dtype 获取数据类型",arr1.dtype)    #数据类型保存在dtype对象中
arr1 = np.array([1, 2, 3], dtype=np.int8)
print("# 创建数表时可以解释元素为特定数据类型", arr1)

data2=[[1, 2, 3, 4], [5, 6, 7, 8]]    #创建嵌套序列（由等长列表组成的列表）
arr2=np.array(data2)    #转换为多维数组
print("np.array(List<List>)转换为二维数组")
print(arr2)

print("# zeros(int)创建指定长度(10)的全0数组\n", zeros(10))
print("# 如果要二维的话需要一个shape(int,int)\n", zeros((10, 10)))
print("# 创建指定长度的(3行6列二维)的全1数组\n", ones((3, 6)))

print("内置函数range(10):", range(10))    #创建指定数量的顺序列表（内置函数,默认0开始）
print("numpy函数arange(10):", arange(10))    #创建指定数量的顺序数组
